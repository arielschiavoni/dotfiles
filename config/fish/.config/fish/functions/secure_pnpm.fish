function secure_pnpm --description "Secure pnpm wrapper that scans for vulnerabilities before running scripts"
    ...
    # Check if --skip-trivy flag is present in arguments
    set -l skip_trivy false
    if contains -- --skip-trivy $argv
        set skip_trivy true
        # Remove --skip-trivy from arguments to avoid passing it to pnpm
        set -l new_args
        for arg in $args
            if test "$arg" != --skip-trivy
                set -a new_args $arg
            end
        end
        set args $new_args
    ...
    if test "$cmd" = install; or test "$cmd" = i; or test "$cmd" = add
        # Check if --skip-trivy flag was provided
        if $skip_trivy
